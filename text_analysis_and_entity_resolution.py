# Databricks notebook source exported at Thu, 15 Sep 2016 18:22:19 UTC
# MAGIC %md
...
def tokenize(string):
    ...
    """
    return [token for token in re.split(split_regex,string.lower()) if (token != "" and token not in stopwords)]
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.sortBy(lambda x: len(x[1]), ascending = False).take(1)
...
def idfs(corpus):
    ...
    """
    uniqueTokens = corpus.map(lambda (k,v):list(set(v)))
...
#idfsToCountRow = idfsSmall.map(lambda s: s[1]).collect()
